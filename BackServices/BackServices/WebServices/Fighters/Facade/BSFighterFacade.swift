...
final public class BSFighterFacade: BSBaseFacade {
    ...
    public func getRankings() {
        do {
            let request = try getRequest(uri: "fighter/rankings")
            connection.delegate = self
            connection.sendRequest(request: request, requestName: String(describing: BSRankingResponse.self))
        } catch let error {
            print(error)
        }
    }
...
extension BSFighterFacade: BSConnectionDelegate {
    public func recievedData(data: Data, requestName: String) {
        ...
        case  String(describing: BSRankingResponse.self):
            decodeEntity(responseType: [BSRankingResponse].self, data: data, requestName: requestName)
        default: break
        }
    }
}
